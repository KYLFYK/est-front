...
export const LandApi = {
  getAllLand: async (skip = 0, take = 10) => {
    try {
      return await instance.get(`${UrlObj.land}`, {
  ...
  getLandById: async (id: number) => {
    try {
      ...
      let object_specsGuide:
        ...
        .map((guid) => sortGuide(guid, guid.subtitle_ru))
        ...
        legalPurityData: {
          ...
          tabsData: {
            ...
            recomendations: [
              {
                value:
                  "Квартира меняла владельцев несколько раз за последние 3 года",
                label:
  ...
  legalPurityData: {
    ...
    tabsData: {
      general: Array<{
        value: string;
        description: string;
        label: Array<{ title: string; text: string }>;
      }>;
      founders: Array<{
        value: string;
        description: string;
