...
import { getInitialStateLegalPurityTab, isValidLegalPurityDetailsTab, TLegalPurityTabState } from "../../lib"
...
const LegalPurityDetails: React.FC<Props> = observer(({ onNextTab, onPrevTab, objectType }) => {
    ...
    const isValidFloors = ("floors" in realEstateRegister) && !!realEstateRegister.floors.length


    const onChangeInputs = (event: React.ChangeEvent<HTMLInputElement>, valueField: keyof TLegalPurityTabState['realEstateRegister']) => {
        setValues({ ...values, realEstateRegister: { ...values.realEstateRegister, [valueField]: event.target.value } })
    }
