...
const ContactOffice: FC<ContactOfficeType> = ({
  contactOffice,
  link,
  destination,
}) => {
  ...
  return (
    <div className={css.contactOfficeBlock}>
          ...
          co.href ? (
            ...
            >
              <Typography key={index}>
                ...
                >
                  {searchIcon(co.title)}
                  <div className={css.margin}>{co.value}</div>
                </div>
              </Typography>
            </a>
          ) : (
            <>
              ...
              {index === 0 && (
                <a
                  href={
                    `https://www.google.com/maps/@${destination.lat},${destination.lng},11z`
                  }
                  className={css.mobileLink}
                  target="_blank"
                >
                  <Typography color={"nude"}>Открыть Google карты</Typography>
                </a>
              )}
            </>
          )
      ...
      <div className={css.link}>
        <Link href={link}>
          <a
            href={`https://yandex.by/maps/?ll=${destination.lng}%2C${destination.lat}&mode=routes&rtext=${currentLocation?.coords.latitude}%2C${currentLocation?.coords.longitude}~44.962107%2C34.104277&rtt=auto&ruri=~&z=7`}
            style={{ textDecoration: "none" }}
            target={"_blank"}
            rel={"noreferrer"}
          >
            <Typography color={"nude"} weight={"bold"}>
              Проложить маршрут до офиса
            </Typography>
          </a>
        </Link>
      </div>
