...
export const Breadcrumbs: React.FC<BreadcrumbsPropsType> = observer(
  ({ className, items, text, location, variant = "primary" }) => {
    ...
    const onReturntoSearch = () => {
      FILTER_ACTIONS_OPTIONS.filter(
        (a: any) => a.label === breadCrumbsStore.get()[1].split(" ")[0]
      )[0] &&
        searchStore.setOrderType(
          FILTER_ACTIONS_OPTIONS.filter(
            (a: any) => a.label === breadCrumbsStore.get()[1].split(" ")[0]
          )[0].value
        );
      ...
      if (
        FILTER_HOUSE_TYPES.filter(
          (a: any) => a.label === breadCrumbsStore.get()[1].split(" ")[1]
        )[0]
      ) {
        searchStore.setHouseType(
          FILTER_HOUSE_TYPES.filter(
            (a: any) => a.label === breadCrumbsStore.get()[1].split(" ")[1]
          )[0].value
        );
        ...
        {breadCrumbsStore.get().map((i: string, id: number) => {
          return (
            <div key={id} className={s.item}>
              ...
              >
                <div
                  onClick={location === "object" ? onReturntoSearch : () => {}}
                >
                  {i}
                </div>
              </Typography>
