...
export const CompareInput: React.FC<Props> = ({ classNameWrapper, location, classNameInput, classNameInputFrom, classNameInputTo, Icon, valueFrom = '', valueTo = '', placeholderFrom, placeholderTo, onChangeFrom, onChangeTo }) => {
    return (
        <div className={classNames(s.wrapper, classNameWrapper)}>
            <input type='number' className={classNames(s.input, location === 'planning' ? classNameInput : classNameInputFrom)} placeholder={placeholderFrom} value={valueFrom} onChange={(event => onChangeFrom(event.target.value))} />
            <div className={s.divider} />
            <input type='number' className={classNames(s.input, location === 'planning' ? classNameInput : classNameInputTo)} placeholder={placeholderTo} value={valueTo} onChange={(event => onChangeTo(event.target.value))} style={{
