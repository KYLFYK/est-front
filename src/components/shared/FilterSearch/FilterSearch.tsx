...
const FilterSearch :FC<FilterSearchType>= ({type='agent'}) => {
        ...
        <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '10px 0'}}>
            ...
            <div style={{display: 'flex'}}>
                ...
                <Upload onClick={upload}/>
                {
                    ...
                    && <>
                        <Oval onClick={oval}/>
                        <VidMenu onClickMap={onClickMap} onClickTable={onClickTable} active={active}/>
                    </>
                }
                {
                    type === 'owner'
...
const VidMenu: FC<VidMenuType> = ({onClickMap, onClickTable, active}) => {
    return (
        <div style={{border:'1px solid #f2f2f2', borderRadius:'8px',height:"38px",margin:'0 4px',minWidth:"83px"}}>
            <button style={{border:'none',background:"rgba(0,0,0,0)",padding:'6px 8px'}} onClick={onClickTable}>
                <FilterIconTable active={active}/>
            </button>
            <VerticalIcon/>
            <button style={{border:'none',background:"rgba(0,0,0,0)",padding:'6px 8px'}} onClick={onClickMap}>
...
const PrinterIcon: FC<OnClickType> = ({onClick}) => {
    return (
        <span onClick={onClick}>
            <svg style={{margin: "0px 4px", cursor: 'pointer'}} width="40" height="40" viewBox="0 0 40 40" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
                <rect x="0.5" y="0.5" width="39" height="39" rx="7.5" fill="white" stroke="#F2F2F2"/>
                <path
