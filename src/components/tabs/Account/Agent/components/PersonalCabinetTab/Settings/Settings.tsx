...
const PersonalCabinetSettings: FC<PersonalCabinetSettingsType123> = observer(
  () => {
    ...
    const [valuePhones, setValuePhones] = useState<Array<string>>(
      personalCabinet.phones
    );
    const [valueLogin, setValueLogin] = useState<string>(personalCabinet.login);
    const [valuePasswordOld, setValuePasswordOld] = useState<string>(
      personalCabinet.passwordOld
    );
    const [valuePasswordNew, setValuePasswordNew] = useState<string>(
      personalCabinet.passwordNew
    );
    const [valueMessagePhone, setValueMessagePhone] = useState<string>(
      personalCabinet.messagePhone
    );
    const [valueMessageEmail, setValueMessageEmail] = useState<string>(
      personalCabinet.messageEmail
    );
    ...
    const save = async () => {
      const updateDate: UpdateAgentCabinetType = {
      ...
      <div>
        <div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center" }}>
              <div
                style={{ cursor: edit ? "pointer" : "", padding: "0 15px" }}
                onClick={() => {
                  if (edit) createPhone();
                }}
              >
                <Typography>+ Добавить телефон</Typography>
              </div>
              <div>
                <BaseButton
        ...
        <div>
          ...
          <div style={{ display: "flex" }}>
            ...
            <BaseButton className={css.marginButton} type={"secondary"}>
              Сохранить
            </BaseButton>
          </div>
        </div>
        ...
        <div className={css.marginColumn}>
          <BaseButton type={"secondary"} onClick={save} isActive>
            Сохранить
          </BaseButton>
        </div>
      </div>
