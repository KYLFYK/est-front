...
import {typeObjectsVision} from "./GuidesItem";
import BaseButton from "../../../../../shared/BaseButton/BaseButtons";
import Typography from "../../../../../shared/Typography/Typography";
import {BaseDropDown} from "../../../../../shared/BaseDropDown/BaseDropDown";

type AddGuideType={
    onCreate:(type_en:string,value:string,visionObject:Array<string>)=>void
    onExit:()=>void
...
const AddGuide:FC<AddGuideType> = ({onCreate,onExit,option,onDropDown,valueDropDown}) => {

    const [type_en,setType_en]=useState<string>('')
    const [type_ru,setType_ru]=useState<string>('')
    const [value,setValue]=useState<string>('')
    const [forObjects,setForObjects]=useState<Array<string>>([])

    const addVisionObject = (title:string) => {
        if (forObjects.length !== 0) {
            const icon = [...forObjects]
            let deleteMode = false
            for (let x = 0; x < forObjects.length; x++) {
                if (icon[x] === title) {
                    icon.splice(x, 1)
                    deleteMode = true
                }
    ...
    return (
        <div >
            ...
            <div>
                <div className={css.df_mTB5}>
                    <BaseDropDown options={option} value={valueDropDown} onChange={(e)=>onDropDown(e)}  placeholder={'Выберите тип'}/>
                </div>

                {/*<div className={css.df_mTB5}>*/}
                {/*    <BaseInput*/}
                {/*        label={'пример - construction'}*/}
                {/*        value={type_en}*/}
                {/*        style={{width:'300px'}}*/}
                {/*        onChange={e=>setType_en(e.currentTarget.value)}*/}
                {/*    />*/}

                {/*</div>*/}
                {/*<div className={css.df_mTB5}>*/}
                {/*    <BaseInput*/}
                {/*        label={"пример - Тип дома"}*/}
                {/*        value={type_ru}*/}
                {/*        style={{width:'300px'}}*/}
                {/*        onChange={e=>setType_ru(e.currentTarget.value)}*/}
                {/*    />*/}
                {/*</div>*/}
                <div className={css.df_mTB5}>
                    <BaseInput
                        label={"пример - Монолитный"}
                        value={value}
                        style={{width:'300px'}}
            ...
            <div style={{display: 'flex', flexDirection: 'column'}}>
                {
                    typeObjectsVision.map((type: string, index) => (
                        <VisionCheckboxType
                            key={index}
                            vision={type}
                            onCheckedType={(e) => addVisionObject(e)}
                            guideInfo={forObjects}
                            index={index}
                            disable={true}
                        />
                    ))
            ...
            <div style={{display: 'flex',marginTop:'10px'}}>
                <div style={{marginRight:"10px"}}>
                    <BaseButton
                        type={"secondary"}
                        onClick={()=>onCreate(valueDropDown,value,forObjects)}
                        isActive
                    >
                        Сохранить
                    </BaseButton>
