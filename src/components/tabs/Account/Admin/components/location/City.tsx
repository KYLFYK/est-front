...
const City = observer(() => {
  ...
  const [activeCityObject, setActiveCityObject] = useState<{
    id: number;
    name: string;
  ...
  const [valueOptionDropDown, setValueOptionDropDown] = useState<
  ...
  useEffect(() => {
    setValueOptionDropDown(
      region.map((r: any) => ({ value: r.id, label: r.name }))
    );
  }, [region]);
  ...
  const onEditCity = async () => {
    ...
    await editCity(activeCityObject.id, {
      regionId: +valueDropDown,
      name: valueCity,
    });
  };
  ...
  return (
    <div className={css.df_jc}>
        ...
        {loaded
          ? city.map((reg: any, index: number) => (
              <div key={reg.id} className={css.df}>
                ...
                <div
                  onClick={() => onActiveEditCity(index)}
      ...
      <div onClick={() => setAddModalCity(true)}>
        <BaseButton icon={<AddIcon />}></BaseButton>
      </div>
      {
        <Modal
        ...
        >
          ...
          <div>
            ...
            <div className={css.df_mT_10}>
              ...
              <BaseButton onClick={cancelEditCity}>отмена</BaseButton>
            </div>
          </div>
        </Modal>
      }
      {
        ...
        >
          ...
          <div className={css.df_mT_10}>
            ...
            <BaseButton onClick={cancelAddCity}>Отмена</BaseButton>
          </div>
        </Modal>
      }
    </div>
