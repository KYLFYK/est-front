...
const Region = observer(() => {
    ...
    const [valueRegion, setValueRegion] = useState<string>('')
    const [editId, setEditId] = useState<number>(0) // id Region for Edit
    ...
    const cancelAddRegion = () => {
        setAddModalRegion(!addModalRegion)
    }

    const onAddRegion = async() => {
        setAddModalRegion(!addModalRegion)
        await addRegion(valueRegion)
    }

    const onEditRegion = async(id:number,name:string) => {
        setEditId(id)
        setValueRegion(name)
        setEditModalRegion(!addModalRegion)
    }

    const saveEditRegion = async () => {
        await editRegion(editId,valueRegion)
        setValueRegion('')
        setEditModalRegion(!editModalRegion)

    }
    const backEditRegion = () => {
        setValueRegion('')
        setEditModalRegion(!editModalRegion)
    ...
    return (
        <div className={css.df_jc}>
            <div>
                {
                    loaded ? region.map((reg: any) => (
                        <div key={reg.id} className={css.df}>
                            ...
                            <div
                                onClick={() => onEditRegion(reg.id,reg.name)}
            ...
            {
                <Modal
                    setActive={() => setEditModalRegion(!editModalRegion)}
                    active={editModalRegion}
                >
                    <div>
                        ...
                        <Typography className={css.mT_20} weight={"bold"}>
            ...
            {
                <Modal
                    setActive={() => setAddModalRegion(!addModalRegion)}
                    active={addModalRegion}
                >
                    <div>
                        <svg width="162" height="20" viewBox="0 0 162 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd" clipRule="evenodd"
