...
const CardObjectSaveSearch: FC<CardObjectSaveSearchType> = ({
  id,
  nameObject,
  locations,
  newAds,
  ...
  const alertAdsSearch = (status: string) => {
    switch (status) {
      case "none":
        return "Не получать оповещения";
      case "hour":
        return "Получать каждый час";
      case "day":
        return "Получать каждый день";
      case "daily":
        return "Получать еженедельно";
      default:
        return "Не получать оповещения";
    }
  };
  ...
  return (
    <div className={css.card} key={id}>
      <div className={css.df_jc}>
        ...
        <FavoriteOffIcon onClick={(id) => ""} id={id} />
      </div>
