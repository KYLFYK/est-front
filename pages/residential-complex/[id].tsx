...
const ResidentialComplex: NextPage = observer((props: any) => {
  ...
  const getObjectId = async () => {
    ...
    const res = await instanceV2
      .get(
        `https://estat.101bot.ru/api/v1/reality-objects/7e8b2919-0a15-4d03-8db6-ec2ae0c14533`
      )
      .then((e) => e.data);
    ...
    const objectComplex = {
      ...
      info: res.params
        .map((general: any) => {
          if (
            general.reality_object_param.reality_object_param_group.name_rus ===
    ...
    const objectTeritori = {
      ...
      specificationsItems: res.params
        .map((r: any) => ({
          label: {
            title:
              r.reality_object_param.reality_object_param_subgroup?.name_rus ===
              "Объекты на территории жилого комплекса"
                ? r.reality_object_param.name_rus
                : "",
            text: "",
  ...
  useEffect(() => {
    ...
    breadCrumbsStore.addBreadCrumbs(`ЖК`, 1);
          ...
          <div ref={general}>
            <GeneralInfo
              ...
              images={
                props.images
                  ? props.images.map(
                      (el: { id: number; url: string }) => el.url
                    )
                  : IMAGES_SET
              }
